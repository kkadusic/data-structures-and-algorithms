...
string Test_BrojElemenata(){
    ...
    if (s.brojElemenata() != 5) return "Test neuspjesan";
    return "Test uspjesan";
}

string Test_Brisi(){
    Stek<int> s;
    for (int i=0; i<100; i++) s.stavi(i);
    s.brisi();
    if (s.brojElemenata() == 0) return "Test uspjesan";
...
string Test_Stavi_Vrh(){
    ...
    s.stavi(-10);
    s.stavi(10);
    if (s.vrh() == 10) return "Test uspjesan";
    return "Test neuspjesan";
}
...
string Test_Dodjela(){
    Stek<int> s1;
    Stek<int> s2;
    for (int i=0; i<5; i++) s1.stavi(i);
    for (int i=10; i<20; i++) s2.stavi(i);
    int x = s2.brojElemenata();
    s2 = s1;
    int y = s2.brojElemenata();
    while (s2.brojElemenata()) s2.skini();
    if (x == 10 && y == 5 && s2.brojElemenata() == 0) return "Test uspjesan";
    return "Test neuspjesan";
}
