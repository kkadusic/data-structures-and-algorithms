...
public:
    ...
    Tip &operator[](int i) const {
        if (i < 0 || i >= duzina)
            throw ("Pristup indeksu van opsega");
        return *(niz[i]);
    }
};
...
int main() {
    try {
        ...
        cout << left << setw(60) << "Test dodavanja elemenata (JednostrukaLista):" << right << Test_Dodavanje(&l)
             << endl;
        ...
        cout << left << setw(60) << "Test svih metoda (tip string, NizLista):" << right
             << Test_SveMetodeKodTipaString(&n1) << endl;
        cout << left << setw(60) << "Test svih metoda (tip string, JednostrukaLista):" << right
             << Test_SveMetodeKodTipaString(&l1);
    }
    catch (...) {
