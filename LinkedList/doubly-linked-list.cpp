...
public: 
    ...
    DvostrukaLista<Tip>& operator= (const DvostrukaLista<Tip> &l){
        if (this == &l) return *this; //zbog destruktivne samododjele
        if (this -> brojElemenata() != 0) this->obrisi();
        tekuci = nullptr;
        prvi = nullptr;
        zadnji = nullptr;
        duzina = 0;
...
public:
    ...
    bool prethodni(){
        if (pok->prethodni == nullptr) return false;
        pok = pok->prethodni;
        return true;
    }
    bool sljedeci(){
...
int main (){
    try {
        ...
        cout << left << setw(60) << "Test svih metoda za tip string:" << right << Test_SveMetodeKodTipaString(&l1) << endl;
    }
    catch(...){
