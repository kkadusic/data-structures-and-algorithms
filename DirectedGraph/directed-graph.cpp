...
public: 
    ...
    void postaviBrojCvorova(int brojCvorova) override {
        if (brojCvorova < vektorPokCvor.size()) throw "Error";
        mat.resize(brojCvorova);
    ...
    void obrisiGranu(int polazniCvor, int dolazniCvor) override {
        ...
        for (int i=0; i<itMax; i++){
            if (vektorPokGrana[i]->dajPolazniCvor() == *vektorPokCvor[polazniCvor] && vektorPokGrana[i]->dajDolazniCvor() == *vektorPokCvor[dolazniCvor]){
