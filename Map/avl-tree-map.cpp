...
public:
    ...
    virtual const TipVrijednosti& operator[](const TipKljuca& k) const = 0; //Moglo se koristiti i bez prvog const i ref.,
...
class AVLStabloMapa : public Mapa<TipKljuca, TipVrijednosti> {
    struct Cvor {
        ...
        lijevo(l), desno(d), roditelj(r), balans(0){}
    };
...
public:
    ...
    AVLStabloMapa(const AVLStabloMapa<TipKljuca, TipVrijednosti>& a) : korijen(nullptr), brojEl(0), prazan(TipVrijednosti()) {
...
public: 
    ...
    BinStabloMapa(const BinStabloMapa<TipKljuca, TipVrijednosti>& b) : korijen(nullptr), brojEl(0), prazan(TipVrijednosti()){
