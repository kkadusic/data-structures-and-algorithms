...
public:
    ...
    HashMapa(const HashMapa<TipKljuca, TipVrijednosti> &mapa) : v(new TipVrijednosti *[mapa.kapacitet]),
                                                                k(new TipKljuca *[mapa.kapacitet]), brojEl(mapa.brojEl),
                                                                kapacitet(mapa.kapacitet), pF(mapa.pF) {
...
class NizMapa : public Mapa<TipKljuca, TipVrijednosti> {
    ...
    // Na kraju ovaj "trik" nije iskoristen u ovoj pripremi, jer const oper. u Mapa nije const TipVrijednosti&
...
class BinStabloMapa : public Mapa<TipKljuca, TipVrijednosti> {
    ...
    Cvor *trazi(const TipKljuca &k, Cvor *c) const { // Krecemo pretragu od cvora c
        if (c == nullptr)
            return 0; // Kljuc se ne nalazi u stablu
        if (k == c->kljuc)
            return c;
        if (k < c->kljuc)
